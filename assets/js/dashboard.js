...
async function renderDashboard(indexJsonPath, mountId){
  ...
  function sitePath(p){
    ...
    const a = document.createElement('a'); a.href = p; return a.pathname;
  }
  ...
  const table = document.createElement('table'); table.className = 'trial-balance';
  table.innerHTML = `<thead><tr>
    <th style="text-align:left">章 / セクション</th>
    <th>進捗</th><th>達成率</th><th>最終更新</th><th>操作</th></tr></thead>`;
  const tbody = document.createElement('tbody');

  function lsKey(qid, suffix){ return `quiz:${qid}${suffix ? ':'+suffix : ''}`; }
  function fmtDate(ms){
    if (!ms) return '-';
  ...
  btnAll.onclick = () => {
    ...
    keys.forEach(k => { if (k.startsWith('quiz:')) localStorage.removeItem(k); });
    location.reload();
  };
  ...
  for (const q of quizzes){
    ...
    const tdTitle = document.createElement('td'); tdTitle.style.textAlign='left'; tdTitle.textContent = q.title || q.quizId;
    const tdProg  = document.createElement('td'); tdProg.textContent = `${keyScore} / ${total || '?'}`;
    const tdPct   = document.createElement('td'); tdPct.textContent  = `${pct}%`;
    const tdUpd   = document.createElement('td'); tdUpd.textContent  = fmtDate(updatedAt);

    const tdOps   = document.createElement('td');
    const aOpen = document.createElement('a'); aOpen.href = pageHref; aOpen.textContent = '開く';
    const btnInfo = document.createElement('button'); btnInfo.textContent = '詳細';
    ...
    tdOps.appendChild(aOpen); tdOps.appendChild(document.createTextNode(' / '));
    tdOps.appendChild(btnInfo); tdOps.appendChild(document.createTextNode(' / '));
    tdOps.appendChild(btnReset);

    tr.appendChild(tdTitle); tr.appendChild(tdProg); tr.appendChild(tdPct); tr.appendChild(tdUpd); tr.appendChild(tdOps);
